...
export class UserService {
   ...
   */
  checkUser() {
    const userIdStr = localStorage.getItem('username');
    if (userIdStr) {
        return this.getUsuarioPorID(userIdStr).pipe(
          map(user => {
            this.setUser(user);
  ...
  iniciarSesion(username: string, password: string) {
    return this.httpClient.get<User[]>(`${this.apiUrl}?username=${username}&password=${password}`).pipe(
      map(users => {
        if (users.length > 0) {
          return users[0];
        } else {
  ...
  getUsuarioPorID(id: string) {
    return this.httpClient.get<User[]>(`${this.apiUrl}?username=${id}`).pipe(
      map(users => {
        if (users.length > 0) {
          return users[0];
        } else {
